...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        actionsAndDistances = []
        ghostPositions = [beliefs.argMax() for beliefs in \
                          livingGhostPositionDistributions]

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            minDist = min([self.distancer.getDistance(successorPosition, pos) \
                           for pos in ghostPositions])
            actionsAndDistances.append((action, minDist))

        return min(actionsAndDistances, key = lambda x: x[1])[0]
